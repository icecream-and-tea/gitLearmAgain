...
inline T SignExtend(const T x) {
    // Extend the number
    // TO BE DONE
    short int judge  = (0x1 << (B - 1));//判断最高位
    short int temp = ~((0x1 << B ) - 1); //高位全变为1，低位为0;
    if(judge & x)
        return x | temp; //最高位为1
    else
        return x;//最高位为0
...
void virtual_machine_tp::VM_JSR(int16_t inst) {
    ...
    //创建一个临时存放PC的变量，最后将该值放入R7;
    int16_t temp_pc = reg[R_PC];                                    //不确定
    if (gIsDetailedMode) {
        std::cout << reg[R_PC] << std::endl;
        std::cout << pc_offset << std::endl;
    }
    if(flag){
        ...
        //JSRR
        int baseR = (inst >> 6) & 0x7;
        reg[R_PC] = reg[baseR] + pc_offset;
    }
    reg[7] = temp_pc;//将PC存入R7
}
...
void virtual_machine_tp::VM_LDI(int16_t inst) {
    ...
    int16_t dr = (inst >> 9) & 0x7;
    int16_t pc_offset = SignExtend<int16_t, 9>(inst & 0x1FF);
    int16_t temp_mem = mem[reg[R_PC] + pc_offset];
    reg[dr] = mem[temp_mem];
    UpdateCondRegister(dr);
}
...
void virtual_machine_tp::VM_STI(int16_t inst) {
    ...
    int16_t sr = (inst >> 9) & 0x7;
    int16_t pc_offset = SignExtend<int16_t, 9>(inst & 0x1FF);
    int16_t temp_mem = reg[R_PC] + pc_offset;
    mem[temp_mem] = reg[sr];
...
void virtual_machine_tp::VM_TEMP(int16_t inst) {
    // DONE
    int flag = inst & 0b100000;
    int dr = (inst >> 9) & 0x7;
    int sr1 = (inst >> 6) & 0x7;
